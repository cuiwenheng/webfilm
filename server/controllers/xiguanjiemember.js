...
function save(body) {
    ...
    // 	}
    // }
    return mysql('xiguanjiemember').count('open_id as hasMember').where({
        open_id
    })
    .then(res => {
        // 如果存在用户则更新
        if (res[0].hasMember) {
            return mysql('xiguanjiemember').update(memdata).where({
                open_id
            })
        } else {
        	// memberdata.open_id=open_id;
            return mysql('xiguanjiemember').insert(memdata)
        }
    })
    .then(() => ({
        open_id: open_id
...
function getxiguanjiemember(openId){
    return mysql('xiguanjiemember').select('*').where({ open_id: openId }).first();
}
// 用户信息接口
module.exports = {
    // 小程序请求 websocket 地址
    get: async ctx => {
        ...
        let data = await getxiguanjiemember(openId)
        ctx.state.data =data;
    },
