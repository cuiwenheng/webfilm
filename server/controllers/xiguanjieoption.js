...
function save(body) {
    ...
    // 	}
    // }
    return mysql('xiguanjiequestion').count('open_id as hasMember').where({
        open_id:open_id,thedate:body.thedate
    })
    .then(res => {
        // 如果存在用户则更新
        if (res[0].hasMember) {
            return mysql('xiguanjiequestion').update(body).where({
                open_id:open_id,thedate:body.thedate
            })
        } else {
        	// memberdata.open_id=open_id;
            return mysql('xiguanjiequestion').insert(body)
        }
    })
    .then(() => ({
        open_id: open_id
...
function getxiguanjieoption(openId){
    return mysql('xiguanjiequestion').select('*').where({ open_id: openId });
}
// 用户信息接口
module.exports = {
    // 小程序请求 websocket 地址
    get: async ctx => {
        ...
        let data = await getxiguanjieoption(openId)
        ctx.state.data =data;
    },
