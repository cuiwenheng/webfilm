...
Page({
  ...
  onLoad: function (options) {
    ...
    if(sess&&sess.userinfo&&sess.userinfo.openId){
      ...
      wx.request({
        url: app.config.service.uploadsmoke, //仅为示例，并非真实的接口地址
        ...
        success: function(res) {
          ...
          if(res.data.data.smokevalue){
            var thelocation=res.data.data.smokevalue*10;
            that.setData({smokevalue:res.data.data.smokevalue,thelocation:thelocation});
          }
        },
        fail:function(res){
          console.log(res);
  ...
  buttonMove: function (e) {
    ...
    var smokevalue=0;
    smokevalue=Math.round(smokevalue+thelocation/10);
    that.setData({
      startPoint:endPoint,
