...
Page({
    ...
    if(sess&&sess.userinfo&&sess.userinfo.openId){
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          if(res.data.data.option4!=undefined){
            that.setData({option4:res.data.data.option4});
          }
        },
        fail:function(res){
          console.log(res);
  ...
  select:function(e){
    this.setData({
         option4:e.target.dataset.value
      })
  },
  goBack:function(e){
    wx.redirectTo({
      url: '../yiyuoption3/yiyuoption3'
    })
  },
  goAhead:function(e){
    if(this.data.option4===undefined){
      this.setData({
        showTopTips:true,
        erromessage:"请选择选项。"
      })
