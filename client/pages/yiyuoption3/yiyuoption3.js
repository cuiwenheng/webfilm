...
Page({
    ...
    if(sess&&sess.userinfo&&sess.userinfo.openId){
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          if(res.data.data.option3!=undefined){
            that.setData({option3:res.data.data.option3});
          }
        },
        fail:function(res){
          console.log(res);
  ...
  select:function(e){
    this.setData({
         option3:e.target.dataset.value
      })
  },
  goBack:function(e){
    wx.redirectTo({
      url: '../yiyuoption2/yiyuoption2'
    })
  },
  goAhead:function(e){
    if(this.data.option3===undefined){
      this.setData({
        showTopTips:true,
        erromessage:"请选择选项。"
      })
