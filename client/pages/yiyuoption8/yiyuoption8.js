...
Page({
    ...
    if(sess&&sess.userinfo&&sess.userinfo.openId){
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          if(res.data.data.option8!=undefined){
            that.setData({option8:res.data.data.option8});
          }
        },
        fail:function(res){
          console.log(res);
  ...
  select:function(e){
    this.setData({
         option8:e.target.dataset.value
      })
  },
  goBack:function(e){
    wx.redirectTo({
  ...
  goAhead:function(e){
    if(this.data.option8===undefined){
      this.setData({
        showTopTips:true,
        erromessage:"请选择选项。"
      })
