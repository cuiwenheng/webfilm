...
Page({
    ...
    if(sess&&sess.userinfo&&sess.userinfo.openId){
      ...
      wx.request({
        ...
        success: function(res) {
          ...
          if(res.data.data.option5!=undefined){
            that.setData({option5:res.data.data.option5});
          }
        },
        fail:function(res){
          console.log(res);
  ...
  select:function(e){
    this.setData({
         option5:e.target.dataset.value
      })
  },
  goBack:function(e){
    wx.redirectTo({
      url: '../yiyuoption4/yiyuoption4'
    })
  },
  goAhead:function(e){
    if(this.data.option5===undefined){
      this.setData({
        showTopTips:true,
        erromessage:"请选择选项。"
      })
